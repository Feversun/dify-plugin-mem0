...
from utils.constants import DELETE_ALL_ACCEPT_RESULT, MAX_PENDING_TASKS_MULTIPLIER
            ...
            if async_mode:
                ...
                if pending_count > client.max_ops * MAX_PENDING_TASKS_MULTIPLIER:
                    logger.warning(
                        ...
                        "rejecting new delete_all operation (user_id: %s)",
                        pending_count,
                        user_id,
                    )
                    error_message = (
                ...
                future = asyncio.run_coroutine_threadsafe(client.delete_all(params), loop)
                client.track_bg_task(future, f"delete_all_memories(user_id={user_id})")
                logger.info(
                    "Delete all memories submitted to background loop "
